...
(function () {
    ...
    function createUI() {
        ...
        const options = [
            ...
            { id: 'order-updatetime-checkbox', text: '按更新时间排列', checked: false },
    ...
    function manipulateWords() {
        const cfg = {
            ...
            orderUpdatetime: document.querySelector('#order-updatetime-checkbox').checked,
        ...
        if (cfg.random && cfg.orderUpdatetime) {
            alert('随机打乱和按更新时间排列不能同时选择!');
            return;
        }
        if (cfg.random) {
        ...
        if (cfg.orderUpdatetime) {
            // 根据 "updatetime": 中的 YYYY-MM-DD HH:mm:ss 来排序
        }
        if (cfg.phrase) {
